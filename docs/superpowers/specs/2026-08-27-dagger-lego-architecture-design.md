...
consumer commits the resolved dependency and language lockfiles. Generated SDK
bindings remain ignored, are regenerated by Dagger from the exact module graph,
and must be reproducible from a clean checkout. Tags may label releases for
humans, but production consumers pin the resolved full SHA.
...
- Clean-checkout generated SDK reproducibility and committed lockfile drift.
...
- Normal rollback is a reviewed PR reverting the shared-module SHA and
  lockfiles, followed by clean regeneration of ignored SDK bindings.
